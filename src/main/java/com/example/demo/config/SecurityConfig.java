    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
            ...
            .authorizeHttpRequests(auth -> auth
                .requestMatchers("/api/auth/google-login", "/api/auth/login-password", "/api/auth/register", "/api/auth/check-email", "/api/top-hotels", "/api/city/{id}/hotel-count", "/api/auth/forgot-password", "/api/auth/reset-password").permitAll() // 允許無需驗證的路徑
    ...
    public WebMvcConfigurer corsConfigurer() {
        return new WebMvcConfigurer() {
            ...
            public void addCorsMappings(CorsRegistry registry) {
                registry.addMapping("/**")
                .allowedOrigins("http://127.0.0.1:5500", "http://localhost:5500")  // 允許前端的域名（根據實際需要修改）

...
public class HotelServiceImpl implements HotelService {
	...
	public Page<HotelSearchRequest> searchHotels(HotelSearchResult dto, int page, int size) {
		...
		List<HotelSearchRequest> filtered = hotels.stream().filter(hotel -> {
			if (dto.getAdult() != null || dto.getChild() != null) {
				int totalPeople = (dto.getAdult() != null ? dto.getAdult() : 0)
						+ (dto.getChild() != null ? dto.getChild() : 0);
				boolean hasRoom = hotel.getRoomTypes().stream().anyMatch(rt -> rt.getCapacity() >= totalPeople);
				if (!hasRoom)
					return false;
			}
			// 城市
			...
			if (dto.getPrice() != null && !dto.getPrice().isEmpty()) {
				...
				Integer minRoomPrice = hotel.getRoomTypes().stream().map(rt -> rt.getPrice().intValue())
			...
			if (dto.getAmenity() != null && !dto.getAmenity().isEmpty()) {
				...
				boolean hasAll = userAmenityIds.stream().allMatch(fac -> hotel.getRoomTypes().stream()
						.flatMap(rt -> rt.getAmenities().stream()).anyMatch(a -> fac.equals(a.getId())));
				if (!hasAll) {
					return false;
				}
			}
			...
			out.setImgUrl(hotel.getImages().stream().filter(img -> Boolean.TRUE.equals(img.getIsCover()))
			...
			out.setRoomType(hotel.getRoomTypes().stream().map(rt -> rt.getRname()).findFirst().orElse(""));
			out.setPrice(hotel.getRoomTypes().stream().map(rt -> rt.getPrice().intValue()).min(Integer::compareTo)
					.orElse(0));
			out.setRating(hotel.getReviews().stream().mapToDouble(r -> r.getScore()).average().orElse(0.0));
		...
		List<HotelSearchRequest> pagedList = fromIndex >= filtered.size() ? java.util.Collections.emptyList()
	...
	public HotelDetailDTO getHotelDetail(Long hotelId) {
		...
		// 查飯店圖片
		List<ImageDTO> imageDTOs = imageRepository.findByHotelIdAndIsCover(hotelId, false).stream().map(img -> {
		...
		// 查所有房型
		List<RoomTypeDTO> roomTypeDTOs = roomTypeRepository.findByHotelId(hotelId).stream().map(roomType -> {
			...
			// 查房型圖片
			List<ImageDTO> roomImages = imageRepository.findByHotelIdAndIsCover(hotelId, true).stream().map(img -> {
				ImageDTO imgDTO = new ImageDTO();
				imgDTO.setId(img.getId());
				imgDTO.setImgUrl(img.getImgUrl());
				imgDTO.setIsCover(img.getIsCover());
			...
			// 查房型設施
			List<AmenityDTO> rtAmenities = amenityRepository.findByRoomTypeId(roomType.getId()).stream().map(a -> {

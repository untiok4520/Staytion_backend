...
public class HotelService {
    ...
    public List<HotelSearchRequestDTO> searchHotels(HotelSearchResultDTO resultDTO) {
        ...
        return hotels.stream()
            ...
            .sorted((h1, h2) -> {
                ...
                if ("price_highest".equals(sort)) {
                    int p1 = h1.getRoomTypes().stream().map(rt -> rt.getPrice().intValue()).min(Integer::compareTo).orElse(0);
                    int p2 = h2.getRoomTypes().stream().map(rt -> rt.getPrice().intValue()).min(Integer::compareTo).orElse(0);
